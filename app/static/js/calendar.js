...
function registerCourse(id, classname, starttime, endtime, startdate, address, type, price) {
    fetch('/api/course', {
        method: 'post',
        body: JSON.stringify({
            "id": id,
...
function sucessRegisterNofication(tongtienthanhtoan){
    var formattedFloatWithSeparator = tongtienthanhtoan.toLocaleString('en-US', {
        minimumFractionDigits: 3,
        maximumFractionDigits: 3
    });
    //console.log(formattedFloatWithSeparator);  // Output: '1,234.57'
        alert('CHÚC MỪNG BẠN ĐÃ HOÀN THÀNH VIỆC ĐĂNG KÝ KHÓA HỌC'
                        + '\n\n'
                        + 'Đây là tổng tiền thanh toán khóa học ' + formattedFloatWithSeparator + "VNĐ"
                        + '\n\n'
                        + 'HẸN GẶP LẠI BẠN SỚM NHẤT CÓ THỂ'
                    );
}
